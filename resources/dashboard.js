			...
			for(p in this.config){
				...
				if(el){
					...
					for(c = 1; c <= 14; c++){
						...
						if(elinner) elinner.classList.remove('c'+c+'-bg');
					}
					if(this.panels[i].config['class']) this.panels[i].el.classList.add(this.panels[i].config['class']);
						...
						if(row == "all"){
							if(el.type=="images"){
								...
								for(r = data.length-1; r >= mn; r--){
									...
									if(typeof el.colour==="string"){
										if(data[r][el.colour]) colour = data[r][el.colour];
